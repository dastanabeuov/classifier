...
class ImportForXclass
  attr_reader :file, :xclass, :user
  ...
  # This method imported tree data from 'xclass' <= 'xclasses_tree_data'
  def import
    ...
    (2..sheet.last_row).each do |row|
      ...
      if full_code.length == 1
        Xclass.find_or_create_by(full_code: full_code) do |xclass|
          ...
          xclass.full_code = full_code
        end
      elsif full_code.length == 2
        root = Xclass.roots.find_by(code: full_code[0])
        ...
        raise "#{t('services.xclass.not_format')} #{@file}"
      end
    end
  end
  ...
  def open_spreadsheet(file)
    ...
    else raise "#{t('services.xclass.not_type')} #{file.original_filename}"
    end
  end
end